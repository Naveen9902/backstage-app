        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 sm:gap-6 mb-6">
            {[
              ...
              { label: 'Applications', value: totalApps.toString(), sub: 'Received total', icon: <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-[#CD7F32]"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> },
              { label: 'Avg Talent Rating', value: avgRating.toFixed(1), sub: 'Based on applicants', icon: <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[#CD7F32]"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg> },
            ].map((stat, index) => (
              <motion.div
                key={stat.label}
                initial={{ opacity: 0, y: 10 }}
                ...
                className="bg-white rounded-2xl p-4 sm:p-5 flex flex-col justify-between h-32 sm:h-36 border border-gray-100"
                style={{ boxShadow: '-4px 4px 0px rgba(205, 127, 50, 0.9)' }}
              >
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-2xl sm:text-3xl font-semibold mb-0.5">{stat.value}</h3>
                    <p className="text-gray-600 font-medium text-[11px] sm:text-sm leading-tight">{stat.label}</p>
                  </div>
                  <div>{stat.icon}</div>
                </div>
                <p className="text-gray-400 text-[10px] sm:text-xs mt-1 truncate">{stat.sub}</p>
              </motion.div>
            ))}
          </div>
