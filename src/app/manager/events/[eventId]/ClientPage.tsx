...
export default function EventWorkersPage() {
  ...
  const fetchEventData = async () => {
    try {
      const eventRes = await apiFetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/user/events/${eventId}`);
      if (eventRes.ok) {
        setEvent(await eventRes.json());
      }
      
      const staffingRes = await apiFetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/manager/events/${eventId}/staffing`);
      if (staffingRes.ok) {
