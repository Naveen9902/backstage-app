...
export default function EventDetailsPage({ params }: Props) {
    ...
    apiFetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/events/all`)
      ...
      .then(data => {
        if (Array.isArray(data)) {
           setRelatedEvents(data.filter(e => e.id !== id && e.status === 'UPCOMING').slice(0, 5));
        }
      })
      .catch(console.error);
  }, [id]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white font-sans flex justify-center items-center">
        ...
        <div className="relative">
          <div className="sticky top-24 bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            <div className="border-t border-gray-100 pt-6 flex flex-col items-center">
              {userId && isWorker && event.staffingRequests && event.staffingRequests.length > 0 ? (
